...
class Attention(nn.Module):
    ...
    def forward(self, inp, hidden,encoder_output,cell=None):
        ...
        #dot product of output of encoder and attn weights
        ct=torch.einsum("snk,snl->knl", alpha, encoder_output)
        #concat embedding and ct
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, inp, target,teacher_force_ratio,capture=True):
        ...
        #handling the output of encoder
...
decoder=Attention(len(hindivocab),args.hidden_size,args.embedding_size,args.num_layers,len(hindivocab),args.cell_type,args.dropout).to(device)
